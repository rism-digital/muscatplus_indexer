...
def _get_country_code(record: pymarc.Record) -> str | None:
    ...
    iso_country_code: str | None = to_solr_single(record, "043", "c")
    if iso_country_code:
...
def _get_location(record: pymarc.Record) -> str | None:
...
def _address(address_field: pymarc.Field) -> dict | None:
    d = {
...
def _get_addresses_data(record: pymarc.Record) -> list | None:
