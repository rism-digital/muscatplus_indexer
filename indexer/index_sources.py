...
def _get_sources(cfg: dict) -> Generator[dict, None, None]:
    ...
    sql_query: str = f"""SELECT child.id AS id, child.title AS title, child.std_title AS std_title,
        ...
        (SELECT DISTINCT CONCAT_WS('|:|', wn.id, wn.marc_source) FROM {dbname}.sources_to_work_nodes AS swn LEFT JOIN {dbname}.work_nodes AS wn ON swn.work_node_id = wn.id WHERE swn.source_id = child.id LIMIT 1) AS work_node,
